...
这个新的符号将显示在 UI 库的符号集合中，可以用在你项目中的所有模型中，这样你可以非常容易的将它添加到任何模型当中。
...
Balsamiq mockups 3 是一个专用的视图创建和管理的符号。使用 mockups 视图创建的符号将被在这里显示，并且你也可以在这个视图中创建符号。
...
**提示**：一个符号所属的符号库会被 UI 库和快速添加中被忽略。无论符号属于哪个符号库，你都将看到你的符号列表。
...
**提示**：你可以给符号库本身添加控件，但是这些控件当符号在你的模型中被使用时将不会被显示。像便签这里控件在添加到符号库时能够用于记录符号，不需要再 UI 中显示出来。

## 导入符号

除了在视图中创建符号，你也可以导入在先前版本的 Balsamiq 模型创建的符号。点击导入符号库按钮来打开一个对话框，导入 BMML 格式的文件（模型中使用的格式，比如， To Go）。
...
你可以像使用其他控件一样在模型中符号。你能够从 UI 库中添加他们，或使用快速添加在你的模型中安置符号和其他控件。
...
当你在模型画布中选中一个符号，你将看到两个符号属性出现在**属性查看器**中： Break Apart 和 Edit Source。

![image](images/symbols-inspector.png)

"Break Apart" 将在符号中的空间和选中的符号本身的连接打断，意味着他不再是一个符号（尽管原始符号完好）。你对分解后的符号的任何改变都不会在其他地方更新，你对一个符号的任何改变在他的分解处也都不会被更新。

"Edit Source" ，它会去做你期望他做的事情，它提供给你可以编辑符号的符号视图。可重复使用的符号的主要好处是，如果你需要作出更改，你只需要在一个地方进行更改即可，它会传播到所有的符号实例（使用中的）中去。当你完成编辑符号时，你可以点击画布上的 "Back to Mockups" 按钮。对符号的编辑将会立即显示在你使用的模型中。
...
例如想象一个你创建主页或模板时的样本,包含一个web页面的标题和导航。
...
符号通过允许你在每一次使用符号时覆盖特定的符号属性来实现这一效果。
为此,首先双击符号的 "enter it"。虽然这个过程非常类似于编辑一个分组的内容,但你会发现桌面模型会提醒你,你所做的为正在重写一个符号的某些属性。

![image](images/symbols-override.png)

此时，你可以操控在符号中的每一个控件，就好像你在编辑一个分组。当你在重写覆盖符号属性时，一些操作是不能进行的，比如添加，删除或是控件分组。如果你想要[编辑所有的实例化符号](http://support.balsamiq.com/customer/portal/articles/110439#editing)，单击 "Edit" 按钮。
...
在符号视图中除了可以创建符号和符号库之外，他也是一个可以管理和组织你的符号的好工具。
...
重命名符号或是符号库时，你可以在左侧的导航栏面板中双击他的名称，或使用 [文本菜单](http://support.balsamiq.com/customer/portal/articles/109151#contextmenu)。

### 删除符号

你同样可以使用 [文本菜单](http://support.balsamiq.com/customer/portal/articles/109151#contextmenu) 从你的项目中删除符号或是符号库。当你删除符号库时，就像是模型或是图像，删除会将他们已送至垃圾箱，在垃圾箱中你可以永久的删除它们，或者是恢复它们。而从符号库中删除一个符号时，删除意味着永久性的删除。
