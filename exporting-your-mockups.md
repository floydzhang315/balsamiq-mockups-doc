...
导出 PDF 时，选择项 Project > Export to PDF...菜单命令（快捷键：CTRL/CMD + P ）。每一个 Mockups 将会在 PDF 中独立的一页中显示。Mockups 将会和项目中的顺序保持一致，所以在导出前请安排还他们的顺序。
...
使用项目菜单中的 导入/导出 Mockup JSON 命令你可Mockups以快速的从一个项目中拷贝 Mockups 至另一个项目。导出至 Mockups JSON(Project > Export > Mockup to JSON) 命令将拷贝选中的 Mockups 的 Balsamiq Mockups 代码至剪贴板。要将这个 Mockups 创建在一个新的项目中，就要切换至这个项目，并且在 Project > Import 菜单中选择 “Import Mockups JSON…” 并且粘贴复制好的代码。
...
如果你正在寻找我们的插件的导出指南，包括 Google Drive, Confluence 和 JIRA，[请查看 FAQ 页面](http://support.balsamiq.com/customer/portal/articles/721932)
